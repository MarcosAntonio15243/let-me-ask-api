...
export async function generateAnswer(
  ...
  const prompt = `
    ...
    - Se a resposta não for encontrada no contexto, apenas responda que não possui informações suficientes para repsonder;
    - Seja objetivo;
  ...
  if (!response.text) {
    throw new Error('Erro ao gerar resposta pelo gemini.');
  }
